...
# ############################################################
fig,axs=subPlots(plt,[0.5,1,1,1,1],b=0.25,dh=0.01,fach=0.5)
ax=axs[1]
...
for phase in "sd","dn","fn","mn":
    ...
    ax.axhspan(ymin,-3.0,color='red',alpha=0.2)
    ax.axhspan(-3.0,0.0,color='green',alpha=0.2)
    ax.set_ylabel(r'$\log(p)$',fontsize=16)
    #ax.set_title("Wave %s"%(PHASES_DICT[phase][1]),position=(0.5,1.05))

    ax.grid()

    ax.set_xticklabels([])

    ax.text(0.5,0.1,"Wave %s"%(PHASES_DICT[phase][1]),
