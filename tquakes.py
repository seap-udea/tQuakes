...
FIELDS_CSV2DB={
    ...
    'Municipio':'municipio',
    '# Estaciones':'numstations','Rms':'timerms','Gap':'gap',
    'Error Latitud (Km)':'qlaterr',
    'Error Longitud (Km)':'qlonerr','Error Profundidad (Km)':'qdeptherr',
    'Estado':'status',
...
def filterSpikes_Debug(quakes):
    from copy import deepcopy as copy
    from matplotlib import pyplot as plt
    #Range of dates
    js=quakes[:,0]
    Ml=quakes[:,4]
    jdmin=int(quakes[:,0].min())
    jdmax=int(quakes[:,0].max())
    print((jdmax-jdmin)/365.25)
    window=1.0 #days
    Nw=int((jdmax-jdmin)/window)
    Nk,jds=numpy.histogram(quakes[:,0],range=(jdmin,jdmax),bins=Nw)
    jas=(jds[1:]+jds[:-1])/2
    ies=numpy.arange(len(jds))
    Nave=Nk.mean()
    dN=Nk.std()
    Nn=(Nk-Nave)/dN
    print("Period: %f-%f = %f years (Nw=%d)"%(jdmin,jdmax,(jdmax-jdmin)/365.25,Nw))
    print("Average rate: %f +/- %f"%(Nave,dN))
    print("Absolute rates: ",Nk)
    print("Normalized rates: ",Nn)
    cond_acc=Nn>(Nave+1.5*dN)
    Nexcl=Nn[cond_acc]
    jexcl=jds[:-1][cond_acc]
    print("Excluded intervals:",jexcl-jdmin)
    print("Number of rates above average: %d"%len(jexcl))
    
    nquakes=copy(quakes)
    rquakes=[]
    for jd in jexcl:
        print("Excluding quakes corresponding to interval: %f"%(jd-jdmin))

        #Accepted
        cond=(nquakes[:,0]<jd)|(nquakes[:,0]>(jd+1))
        try:nquakes=nquakes[cond]
        except:pass

        #Rejected
        cond=(quakes[:,0]>=jd)&(quakes[:,0]<(jd+1))
        print("\tQuakes to be excluded:",quakes[cond])
        
        if len(rquakes)==0:
            rquakes=quakes[cond]
        else:
            rquakes=numpy.concatenate((rquakes,quakes[cond]))

        print("\tNumber of events to be excluded:%d"%len(quakes[cond]))
        print("\tNumber of total events excluded:%d"%len(rquakes))

    print("Number of events after exclusion: %d"%(len(nquakes)))
    print("Number of rejected events: %d"%(len(rquakes)))

    #Plot
    fig=plt.figure()
    ax=fig.gca()
    ax.step(jas-jdmin,Nk,lw=2)
    #ax.plot(quakes[:,0]-jdmin,quakes[:,4],'ko',ms=5)
    #ax.plot(nquakes[:,0]-jdmin,nquakes[:,4],'o',ms=10,mec='red',mfc='none')
    try:
        ax.plot(rquakes[:,0]-jdmin,rquakes[:,4],'v',ms=10,mec='red',mfc='none')
    except:
        pass
    ax.set_ylim((0,7))
    ax.set_xlim((0,jdmax-jdmin))
    #ax.set_xlim((300,600))
    fig.savefig("rate.png")

    return nquakes

def filterSpikesOld(quakes,window=1.0):
    from copy import deepcopy as copy
    jdmin=int(quakes[:,0].min())
    jdmax=int(quakes[:,0].max())
    Nw=int((jdmax-jdmin)/window)
    ...
    dN=Nk.std()
    Nn=(Nk-Nave)/dN
    cond_excl=Nn>(Nave+1.5*dN)
    Nexcl=Nn[cond_excl]
    jexcl=jds[:-1][cond_excl]
    nquakes=copy(quakes)
    iquakes=numpy.arange(len(quakes))
    for jd in jexcl:
        cond=(nquakes[:,0]<jd)|(nquakes[:,0]>(jd+1))
        try:
            nquakes=nquakes[cond]
            iquakes=iquakes[cond]
        except:pass

    return iquakes


def filterSpikes(quakes,window=1.0):
    from copy import deepcopy as copy
    jdmin=int(quakes[:,0].min())
    jdmax=int(quakes[:,0].max())
    Nw=int((jdmax-jdmin)/window)
