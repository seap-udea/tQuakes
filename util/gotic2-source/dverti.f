c$dverti
c------------------------------------------------------------------
      subroutine dverti (plat  , plon  , qlat  , qlon  , wlat  ,
     +                   wlon  , hload , dvertx, dverty, bang   )
c------------------------------------------------------------------
c
...
c
         ...
         enddo
c
         call greenf (ang4, grna, grnb, grnc, 6)
c
         clat = dcos(qlat)
         cang = dcos(azmt)*angq
         sang = dsin(azmt)*angq
...
c
         ...
         enddo
c
         dvertx = -dvertx*am*1.0d-12
         dverty = -dverty*am*1.0d-12
c
      else                      ! Point load
c
         call greenp (angq, grnq, 6)
c
         dvert  = wlat*wlon*dcos(qlat)*hload*dens*grnq
     +          / (angq*angq*1.0d12)
         dvertx = -dvert*dsin(azmt)
         dverty = -dvert*dcos(azmt)
c
      endif
c
      return
